...
export const fetchSingleUser = async (req, res) => {
    try {
        const user = await userModel.findById({ _id: req.body.user_id }).select("-password")
        ...
        if (user) {
            ...
            res.status(400).json({ msg: "Oops!! could not fetch user", success: false })
        }
...
export const UpdateUser = async (req, res) => {
    try {
        ...
        if (user) {
            ...
            res.status(200).json({ msg: "Your details have been updated successfully", success: true, type: 'success', user: user })

        } else {
            res.status(400).json({ msg: "Could not update your details. Please try again later", success: false, type: 'error' })
        }

    } catch (error) {
        res.status(400).json({ error: error.message, msg: error.message, success: false, type: 'warning' })
...
export const fetchFamilyDetails = async (req, res) => {
    ...
    try {
        ...
        const familyDetails = await familyDetailsModel.find({ _id: { $in: user.familyDetails } })
...
export const addFamilyDetails = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, msg: 'family details added successfully', family_detail: familyDetail })



    } catch (error) {
        return res.status(403).json({ error: error, msg: " could not add family detail. Please try again later" })
